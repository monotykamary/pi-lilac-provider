 ...
 *
 * API key: the stored `lilac` credential in ~/.pi/agent/auth.json wins, then
 * the LILAC_API_KEY environment variable. The script refuses to run without one.
 */
...
function resolveApiKey() {
  ...
  return process.env.LILAC_API_KEY || undefined;
}
...
async function fetchModels() {
  ...
  if (!apiKey) {
    throw new Error('No API key found: no `lilac` credential resolved from ' + AUTH_JSON_PATH + ' and LILAC_API_KEY is not set');
  }
